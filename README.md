...
// limitations under the License.lied. See the License for the specific language governing permissions andlimitations under
// the License.
-->

## CmdArgLibCommandNodeFrame

CmdArgLibCommandNodeFrame is part of the [Command Argument Library](https://github.com/ouser4629/cmd-arg-lib.git).

It provides the `CommandNodeFrame` protocol, which
...
2. Define a struct that conforms to `CommandNodeFrame<StateElement>`, and annotate it with `@main`.
...
A struct that defines a CLI by conforming to `CommandNodeFrame<StateElement>` must define:
...
that show how to use `CmdArgLibCommandNodeFrame`.
